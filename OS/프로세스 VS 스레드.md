...
**장점** : 1개의 프로세스가 죽어도 자식 프로세스 이외의 다른 프로세스들은 계속 실행, 프로세스간 공유하는 자원이 없어서 임계영역문제가 발생하지 않음. 
...
#### **프로세스와 스레드의 차이?** (N사 전화면접)

프로세스는 운영체제로부터 CPU 자원을 할당받는 작업의 단위이다. 그리고 메모리에 올라와 실행되고 있는 프로그램의 인스턴스(독립적인 개체)를 의미한다. Code, Data, Stack, Heap의 구조로 독립된 메모리 영역을 할당받는다.
...
#### **메모리가 어떻게 구성되어 있는지 설명해 주세요.**(N사 전화면접)
